...
import { ETIQUETAS_FUENTE, buscarPorClave, catalogosAparte, ofertas, plataformas } from './datos.js';
    ...
    contenedor.innerHTML = lista.map(curso => `
        <article class="curso-card">
            ...
            ${urlSegura(curso.link)
                ? `<a class="card-link" href="${escaparHTML(urlSegura(curso.link))}" target="_blank" rel="noopener noreferrer">Ir al sitio oficial ↗</a>`
    ...
    contenedor.innerHTML = lista.map(curso => `
        <article class="curso-card curso-card-simple">
            ...
            ${curso.descripcion
                ...
                : ''}
            ${urlSegura(curso.link)
                ? `<a class="card-link" href="${escaparHTML(urlSegura(curso.link))}" target="_blank" rel="noopener noreferrer">Ver más en mendoza.edu.ar ↗</a>`
                : '<span class="card-link card-link-muted">Sin link oficial</span>'}
        </article>`).join('');
}
    ...
    contenedor.innerHTML = lista.map(plataforma => `
        <article class="platform-card">
            ...
            ${urlSegura(plataforma.url)
                ? `<a class="platform-link" href="${escaparHTML(urlSegura(plataforma.url))}" target="_blank" rel="noopener noreferrer">Ver oferta en ${capSeguro(plataforma.nombre)} ↗</a>`
...
function construirTablaComparar(items) {
    const filas = [
        ...
        ['Sitio oficial', i => urlSegura(i.link || i.url) ? `<a href="${escaparHTML(urlSegura(i.link || i.url))}" target="_blank" rel="noopener noreferrer">Ir ↗</a>` : '—']
    ];
...
export function renderizarTarjetas(resultados, { mostrarMatch = false, encabezado = '' } = {}) {
    ...
    contenedor.innerHTML = encabezado + resultados.map(oferta => `
        <article class="card">
            ...
            ${urlSegura(oferta.link) ? `<a class="card-link" href="${escaparHTML(urlSegura(oferta.link))}" target="_blank" rel="noopener noreferrer">Ir al sitio oficial ↗</a>` : '<span class="card-link card-link-muted">Formación online</span>'}
            <div class="card-actions">
